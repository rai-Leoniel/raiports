...
//
// Search "R1A01 REAL DATA" for the existing Account Activity By ID wiring
// (unchanged from before).
//
// Search "R1A03 REAL DATA" for everything new in this pass:
//
// 1. Added `CashPositionRow` type alias (same shape as `AccountActivityRow`
//    — both come from the same style of backend response: date/reference/
//    description/debit/credit/running_balance).
//
// 2. Added `ChartOfAccount` type + reused the existing
//    `/approval/reports/chart-of-accounts/` endpoint (already built for
//    R1A01) to populate a REAL Account dropdown for R1A03. Unlike R1A01,
//    the Cash_Position_CIB backend view requires `account_id` — it 400s if
//    missing — so this dropdown has no "All" option and Preview is
//    disabled until an account is chosen.
//
// 3. Added a client-side Financial Year dropdown, built by deduping the
//    existing `/approval/reports/accounting-periods/` endpoint (already
//    built for R1A02) down to distinct `fy` values, plus an "All" option
//    that omits the `fy` param entirely. FLAG: if you'd rather have a
//    dedicated backend endpoint that returns distinct years directly, this
//    block is the only thing that needs to change — swap the effect that
//    builds `fyOptions` for a fetch to that new endpoint.
//
// 4. Added `CashPositionCIBPanel` component — controlled state for
//    Branch/Account ID/Financial Year/Entry Date From-To/Unposted-Posted/
//    Include Previous, calling `/approval/reports/cash-position-cib/` when
//    "Preview" is clicked (no auto-fetch on mount, since Account ID is
//    required and starts unselected — different from R1A01's auto-load).
//    Reuses `buildAccountingPreviewHtml` for rendering, since the response
//    shape is identical to R1A01's.
//
// 5. In the main component's `if (selectedReport)` block: branches to
//    `CashPositionCIBPanel` when `selectedReport.code === "R1A03"`,
//    otherwise falls through unchanged for every other report.
//
// Nothing else in this file was touched — every other report (R1A02,
// R1A04–R1A085, all Inventory reports, all Sales reports) behaves
...
type ReportSection = SubCategory["sections"][number];
type ReportItem = ReportSection["items"][number];
...
// --- R1A01 REAL DATA: local-time date formatter (matches dashboard's fix
// for the UTC-shift bug that silently turned "today" into "yesterday" in
// PHT) ------------------------------------------------------------------
...
// --- R1A01 REAL DATA: Branch dropdown -> real backend branch code -------
// FLAG: only mapping we have confirmed real data for. Confirm this is the
// right mapping before trusting it in production. Reused as-is for R1A03.
const BRANCH_CODE_MAP: Record<string, string> = {
  "head-office": "MAN",
};

// --- R1A01 REAL DATA: shape returned by /approval/reports/account-activity-by-id/
...
// --- R1A03 REAL DATA: /approval/reports/cash-position-cib/ returns the
// identical shape, so this is just a readable alias rather than a
// duplicate type. -----------------------------------------------------
type CashPositionRow = AccountActivityRow;

// --- R1A03 REAL DATA: shape returned by /approval/reports/chart-of-accounts/
type ChartOfAccount = {
  value: string;
  label: string;
};

// --- R1A03 REAL DATA: shape returned by /approval/reports/accounting-periods/
...
export default function CategoryPanel({ category }: { category: Category }) {
  ...
  const defaultSubCategory = category.subCategories[0] ?? null;
  const subCategoryFromUrl = searchParams.get("subcategory");
  const reportFromUrl = searchParams.get("report");
  const search = searchParams.get("search") ?? "";

  const activeSubCategory =
    category.subCategories.find((sub) => sub.key === subCategoryFromUrl) ??
    defaultSubCategory;

  const allItemsInSubCategory: ReportItem[] = useMemo(() => {
    if (!activeSubCategory) return [];
    return activeSubCategory.sections.flatMap((section) => section.items);
  }, [activeSubCategory]);

  const selectedReport =
    allItemsInSubCategory.find((item) => item.code === reportFromUrl) ?? null;

  const filteredSections: { section: ReportSection; items: ReportItem[] }[] =
    useMemo(() => {
      if (!activeSubCategory) return [];

      const q = search.trim().toLowerCase();

      return activeSubCategory.sections.map((section) => {
        const items = !q
          ? section.items
  ...
  const hasAnyResults = filteredSections.some(({ items }) => items.length > 0);
  const showSectionHeaders = (activeSubCategory?.sections.length ?? 0) > 1;
  ...
  const handleReportSelect = (reportCode: string) => {
    updateQuery({
      subcategory: activeSubCategory?.key ?? defaultSubCategory?.key ?? null,
      report: reportCode,
    });
  };
  ...
  const handleExportExcel = () => {
    ...
    const rows = getExportRows({
      ...
      sectionLabel: activeSubCategory?.label ?? category.label,
    });
  ...
  if (selectedReport) {
    // --- R1A01 REAL DATA: branch to the real, dynamic panel. Every other
    // report code falls through to the exact original static behavior
    // below, completely unchanged.
    if (selectedReport.code === "R1A01") {
      return (
        <AccountActivityByIdPanel
          reportName={selectedReport.name}
          sectionLabel={activeSubCategory?.label ?? category.label}
          isDark={isDark}
    ...
    // --- R1A03 REAL DATA: branch to the Cash Position (CIB) panel. Every
    // other report code (including R1A02, R1A04+) still falls through to
    // the original static behavior below, unchanged.
    if (selectedReport.code === "R1A03") {
      return (
        <CashPositionCIBPanel
          reportName={selectedReport.name}
          sectionLabel={activeSubCategory?.label ?? category.label}
          isDark={isDark}
          onBack={handleBackToList}
        ...
        <CardContent className="p-4 sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <h1 className="text-[26px] font-semibold leading-tight tracking-tight text-slate-900 dark:text-white sm:text-[32px]">
                {activeSubCategory?.label || category.label}
              </h1>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {category.subCategories.map((sub) => {
              const isActive = sub.key === activeSubCategory?.key;
          ...
          <CardContent className="flex min-h-[200px] items-center justify-center p-6 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {search.trim()
                ...
                : "No reports added yet for this section."}
            </p>
          </CardContent>
        </Card>
      )}
        ...
        {filteredSections.map(({ section, items }) => {
          ...
          return (
            <div key={section.key} className="space-y-4">
              {showSectionHeaders && (
                <h3 className="px-1 text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  {section.label}
                </h3>
              )}
...
// ============================================================================
// R1A01 REAL DATA: dedicated panel — controlled filters, real fetch, real
// rows rendered into the same visual style as the static accounting preview.
// ============================================================================
function AccountActivityByIdPanel({
  reportName,
  sectionLabel,
  isDark,
  onBack,
  ...
  const fetchReport = async () => {
    ...
    const params = new URLSearchParams({
      branch: BRANCH_CODE_MAP[branch] ?? branch,
      account_id: accountId,
      entry_date_from: dateFrom,
      entry_date_to: dateTo,
      unposted: String(includeUnposted),
      posted: String(includePosted),
    });
    if (journalFrom.trim()) params.set("journal_from", journalFrom.trim());
    if (journalTo.trim()) params.set("journal_to", journalTo.trim());

    try {
      const res = await apiFetch(
        `/approval/reports/account-activity-by-id/?${params.toString()}`,
      );
      ...
      console.error("Account Activity By ID fetch error:", err);
      setRows([]);
  ...
  // Auto-load with today's date on first open — matches "don't have to put
  // something in it" unless tracing back to a different range.
  useEffect(() => {
    fetchReport();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const previewHtml = buildAccountingPreviewHtml(reportName, sectionLabel, isDark, rows);
  ...
  const handleExportExcel = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(
      exportRows.length > 0 ? exportRows : [{ Report: reportName, Section: sectionLabel }],
    );
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "R1A01");
    XLSX.writeFile(workbook, `R1A01-${reportName}.xlsx`.replace(/[\\/:*?"<>|]/g, "-"));
  };
  ...
  const handleClearFields = () => {
    ...
    setDateFrom(today);
    setDateTo(today);
    setIncludeUnposted(true);
    setIncludePosted(true);
  };
          ...
          <div className="grid gap-4 xl:grid-cols-[220px_minmax(0,1fr)_auto]">
            ...
            <div className="flex flex-col gap-2 sm:flex-row xl:flex-col xl:items-end">
              ...
              <Button
                ...
                className={clearFieldsButtonClass}
              >
            ...
            <div className="flex flex-wrap items-center gap-4 pt-1 lg:pt-8">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setIncludeUnposted(checked === true)}
                />
                <label htmlFor="unposted" className="text-sm text-slate-700 dark:text-slate-200">
                  Unposted Entries
                </label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setIncludePosted(checked === true)}
                />
                <label htmlFor="posted" className="text-sm text-slate-700 dark:text-slate-200">
                  Posted Entries
                </label>
              </div>
            </div>
...
// ============================================================================
// R1A03 REAL DATA: Cash Position (CIB) panel — controlled filters, real
// fetch against /approval/reports/cash-position-cib/, real rows rendered
// via the same table markup as R1A01 (identical response shape).
//
// Differences from AccountActivityByIdPanel that are NOT copy/paste bugs:
//   - Account ID is required by the backend, so it's a real dropdown (no
//     "All") sourced from /approval/reports/chart-of-accounts/, and Preview
//     is disabled until one is picked.
//   - Adds a Financial Year dropdown and an "Include Previous" checkbox,
//     both specific to rpt_a103_cib()'s signature.
//   - No auto-fetch on mount, since there's no valid default account to
//     fetch with yet.
// ============================================================================
function CashPositionCIBPanel({
  reportName,
  sectionLabel,
  isDark,
  onBack,
  ...
  // Populate the Account ID dropdown from the same endpoint R1A01 uses.
  useEffect(() => {
    (async () => {
      setAccountsLoading(true);
      try {
        const res = await apiFetch("/approval/reports/chart-of-accounts/");
        if (!res.ok) return;
        const data = await res.json();
        const list: ChartOfAccount[] = Array.isArray(data?.accounts) ? data.accounts : [];
        setAccounts(list);
      } catch (err) {
  ...
  // Build a distinct-years Financial Year dropdown by deduping the same
  // period list R1A02 uses. FLAG: swap this effect for a dedicated
  // distinct-years endpoint if/when one exists.
  useEffect(() => {
    (async () => {
      try {
        const res = await apiFetch("/approval/reports/accounting-periods/");
        if (!res.ok) return;
        const data = await res.json();
        const periods: AccountingPeriod[] = Array.isArray(data?.periods) ? data.periods : [];
        const years = Array.from(new Set(periods.map((p) => p.fy))).sort((a, b) => b - a);
        setFyOptions(years);
      } catch (err) {
        console.error("Accounting Periods fetch error:", err);
      }
    })();
  }, []);
  ...
  const fetchReport = async () => {
    if (!accountId) {
      setError("Please select an Account ID.");
      return;
    }
    ...
    const params = new URLSearchParams({
      ...
      unposted: String(includeUnposted),
      posted: String(includePosted),
      include_previous: String(includePrevious),
    });
    if (fy !== "all") params.set("fy", fy);

    try {
      const res = await apiFetch(
        `/approval/reports/cash-position-cib/?${params.toString()}`,
      );
      ...
      console.error("Cash Position CIB fetch error:", err);
      setRows([]);
      setError("Could not reach the server. Please try again.");
    } finally {
      setLoading(false);
  ...
  const handleExportExcel = () => {
    ...
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "R1A03");
    XLSX.writeFile(workbook, `R1A03-${reportName}.xlsx`.replace(/[\\/:*?"<>|]/g, "-"));
  };
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              <div className="space-y-2">
                ...
                <Select value={accountId} onValueChange={setAccountId}>
                  ...
                  <SelectContent>
                    {accounts.map((acct) => (
                      <SelectItem key={acct.value} value={acct.value}>
                        {acct.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-1 lg:pt-8">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setIncludeUnposted(checked === true)}
                />
                <label htmlFor="cib-unposted" className="text-sm text-slate-700 dark:text-slate-200">
                  Unposted Entries
                </label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setIncludePosted(checked === true)}
                />
                <label htmlFor="cib-posted" className="text-sm text-slate-700 dark:text-slate-200">
                  Posted Entries
                </label>
              </div>
...
// and, as of this pass, R1A03) -> renders real rows. ----------------------
